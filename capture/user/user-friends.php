...
function get_followers($screen_name, $cursor = -1) {
    print "getting followers of $screen_name\n";
    global $twitter_keys, $current_key, $ratefree, $bin_name, $dbh;

    $ratefree--;
    if ($ratefree < 1 || $ratefree % 10 == 0) {
        $keyinfo = getRESTKey($current_key, 'followers', 'list');
        $current_key = $keyinfo['key'];
        $ratefree = $keyinfo['remaining'];
    }
    ...
    if ($tmhOAuth->response['code'] == 200) {
        ...
        echo count($followers) . " users found\n";

        // store in db
        $tr = new TwitterRelations($screen_name, $followers, "followed", $observed_at);
        $tr->save($dbh, $bin_name);

        // continue if there are still things to do
        if (!empty($cursor) && $cursor != -1) {
            sleep(1);
            get_followers($screen_name, $cursor);
        }
    } else {
        $error_code = json_decode($tmhOAuth->response['response'])->errors[0]->code;
        if ($error_code == 130) {
